...
export async function getActivity(userId) {
  try {
    ...
    const repliesAndLikes = await Post.find({
      _id: { $in: childPostIds },
      $or: [
        { author: { $ne: userId } }, // Replies
        { likes: { $ne: userId } }, // Likes
      ],
    }).populate({
      path: 'author likes', // Populate both author and likes
      model: User,
      select: 'name image _id',
    });

    return repliesAndLikes;
  } catch (error) {
